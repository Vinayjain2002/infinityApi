...
const ApplyForAdminController = async (req, res, next) => {
    try {
      ...
      if( existingAdmin && existingAdmin.approvedadmin){
          return res.status(409).json({message: "You are already a Admin"})
      } // Assuming Admin model exists
      else if (existingAdmin) {
        return res.status(405).json({ message: "You are already applied for an admin" }); // Use 409 for conflict
      }
